...
export default function Dashboard() {
    ...
    <div style={{ minHeight: '100vh', background: '#f8f7ff', fontFamily: "'Plus Jakarta Sans', sans-serif", paddingBottom: '8rem' }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .dash-search-input { width: 100%; background: #fff; border: none; border-radius: 14px; padding: 14px 16px 14px 46px; color: #111; font-size: 14px; font-family: inherit; outline: none; box-shadow: 0 8px 32px rgba(0,0,0,0.12); }
        @media (min-width: 600px) { .dash-search-input { border-radius: 16px; padding: 16px 20px 16px 52px; font-size: 15px; } }
        ...
        .song-row-presave { font-size: 10px; font-weight: 700; padding: 4px 10px; border-radius: 100px; flex-shrink: 0; cursor: pointer; border: none; transition: all 0.15s; font-family: inherit; }
  ...
  <button
    ...
    {song.album_id ? 'Ver álbum →' : (isSaved ? 'Guardado ✓' : 'Preguardar')}
  </button>
) : (
                          <>
            ...
            {filtered.songs.length === 0 && filtered.albums.length === 0 && filtered.artists.length === 0 && (
              <div className="empty">
                ...
                <p style={{ margin: 0, fontWeight: '600', fontSize: '14px' }}>Sin resultados{search ? ` para "${search}"` : ''}</p>
