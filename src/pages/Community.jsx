...
export default function Community() {
  ...
  useEffect(() => {
    ...
    if (found) { setOpenPostId(postId); return }
    const fetchPost = async () => {
      const { supabase } = await import('../lib/supabase')
      const { data } = await supabase.from('posts')
        .select('*, post_likes(count), post_comments(count)').eq('id', postId).single()
      if (data) {
        setPosts(prev => { const exists = prev.find(p => p.id === data.id); return exists ? prev : [data, ...prev] })
  ...
  const handlePostDeleted = (postId) => { setPosts(prev => prev.filter(p => p.id !== postId)); setLikedPosts(prev => prev.filter(p => p.id !== postId)) }
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-32">
      ...
      {/* Header */}
      <div className="bg-white border-b border-gray-100 px-4 py-6 sm:px-6 sm:py-10">
        <div className="max-w-3xl mx-auto">
          {/* Título y botón en columna en mobile, fila en desktop */}
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <p className="text-xs font-semibold tracking-widest text-purple-600 uppercase mb-2">Comunidad</p>
              <h1 className="text-3xl sm:text-4xl font-black text-gray-900 mb-2 sm:mb-3">El Blog</h1>
            ...
            {user && (
              <button
                ...
                className="flex items-center justify-center gap-2 bg-purple-700 hover:bg-purple-800 text-white text-sm font-semibold px-4 py-2.5 sm:px-5 sm:py-3 rounded-xl transition shadow-md w-full sm:w-auto shrink-0"
              >
          ...
          {user && (
            <div className="mt-5 flex gap-1">
              <button onClick={() => setTab('feed')}
                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition ${tab === 'feed' ? 'bg-purple-50 text-purple-700' : 'text-gray-500 hover:bg-gray-100'}`}>
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <button onClick={() => setTab('liked')}
                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition ${tab === 'liked' ? 'bg-purple-50 text-purple-700' : 'text-gray-500 hover:bg-gray-100'}`}>
      ...
      {/* Feed */}
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-6 sm:py-8 flex flex-col gap-4">
        {isLoading ? (
          ...
          activePosts.map(post => (
            <PostCard
              ...
              onCommentsOpened={() => { setOpenPostId(null); setSearchParams({}) }}
            />
          ))
        )}
      </div>
