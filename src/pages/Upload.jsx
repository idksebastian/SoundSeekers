...
export default function Upload() {
  ...
  const addCredit = (trackId) => {
    ...
    updateTrack(trackId, 'credits', [...track.credits, { name: track.creditName.trim(), role: track.creditRole }])
  ...
  const validateStep2 = () => {
    ...
    if (isPresave && !releaseDate) return 'Selecciona la fecha de lanzamiento para activar el presave.'
    return null
  }
                ...
                <div className="flex-1 space-y-3">
                  ...
                  <div className="space-y-1">
                    <label className="text-sm font-medium text-gray-700">
                      ...
                      {isPresave && <span className="text-purple-600 text-xs ml-1">(requerida para presave)</span>}
                    </label>
                    <input type="date" value={releaseDate} min={isPresave ? minDate : undefined}
                      onChange={e => setReleaseDate(e.target.value)}
                      className="w-full bg-white text-black border border-gray-300 rounded-xl px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-purple-500 text-sm"/>
                    {isPresave && releaseDate && (
                      <p className="text-xs text-purple-500 font-medium">
                        🔖 El presave estará activo desde hoy hasta el {new Date(releaseDate + 'T00:00:00').toLocaleDateString('es-CO', { day: 'numeric', month: 'long', year: 'numeric' })}
                      </p>
                    )}
                  </div>
                </div>
              ...
              <label className="flex items-start gap-3 cursor-pointer p-3 rounded-xl border border-gray-200 hover:bg-gray-50 transition">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">
                    Los oyentes pueden guardar el lanzamiento antes de que salga. Recibirán una notificación cuando se publique en la fecha que elijas.
                  </p>
                </div>
              </label>
            </div>
          ...
          <div className="space-y-4">
            ...
            {isPresave && releaseDate && (
              <div className="flex items-center gap-3 bg-purple-50 border border-purple-200 rounded-xl px-4 py-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-purple-700">Presave activo</p>
                  <p className="text-xs text-purple-500">
                    Las canciones estarán bloqueadas hasta el {new Date(releaseDate + 'T00:00:00').toLocaleDateString('es-CO', { day: 'numeric', month: 'long', year: 'numeric' })}. Los oyentes que guarden recibirán una notificación al publicarse.
                  </p>
                </div>
              </div>
            )}
