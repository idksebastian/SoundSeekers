...
const moods = [
  ...
  {
    id: 'sad', label: 'Triste',
    icon: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
  },
  {
  ...
  },
  {
    ...
    icon: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m1.636-6.364l.707.707M12 21v-1m0-16a7 7 0 017 7c0 2.5-1.5 4.5-3 6H8c-1.5-1.5-3-3.5-3-6a7 7 0 017-7z"/></svg>
  },
]
...
const weathers = [
  ...
  {
    id: 'cold', label: 'Frío',
    icon: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 3v18M3 12h18M5.636 5.636l12.728 12.728M18.364 5.636L5.636 18.364"/></svg>
  },
  {
...
const MOOD_LABELS = {
  happy: 'Feliz', sad: 'Triste', energetic: 'Energético',
  calm: 'Calmado', nostalgic: 'Nostálgico', focused: 'Concentrado',
}

const WEATHER_LABELS = {
  sunny: 'Soleado', rainy: 'Lluvioso', cloudy: 'Nublado',
  night: 'Noche', cold: 'Frío', warm: 'Cálido',
}
  ...
  const handleGetRecommendations = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8000/recommendations?mood=${selectedMood}&weather=${selectedWeather}`
      )
        ...
        <section className="max-w-3xl mx-auto px-6">
          <div className="bg-purple-50 border border-purple-100 rounded-2xl p-4 mb-6 flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-gray-900">
                Tu playlist — {MOOD_LABELS[selectedMood]} + {WEATHER_LABELS[selectedWeather]}
              </p>
              <p className="text-xs text-gray-400">Curada para tu vibra actual</p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3">
            {songs.map((song, i) => (
              <div
                key={i}
                className="flex items-center gap-4 p-3 rounded-xl bg-white border border-gray-100 shadow-sm hover:shadow-md transition group"
              >
