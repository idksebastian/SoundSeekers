...
import {
  ...
  Heart, Plus, Star, TrendingUp, Music2, ChevronUp
} from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";

// ─── Extracción de color con cancelación y filtro de luminosidad ─────────────
...
// ─── Formatea números grandes: 1200000 → 1.2M ────────────────────────────────
...
export default function Player() {
  ...
  const [albumTracks, setAlbumTracks]     = useState([]);   // tracks del álbum actual
  const [nextSong, setNextSong]           = useState(null); // siguiente en la cola
  ...
  // Controla el pulso de "swipe up" en el mini player
  ...
  // ── Al montar una nueva canción → abrir fullscreen automáticamente ──
  useEffect(() => {
    if (currentSong) setIsFullscreen(true);
  }, [currentSong?.id]);

  // ── Animación de hint en mini player cada 8 segundos ──
  useEffect(() => {
    ...
    // Disparar también al abrir el mini player
    setShowOpenHint(true);
    setTimeout(() => setShowOpenHint(false), 1800);
  ...
  // ── Fetch de datos: artista + álbum + canciones relacionadas ──
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (currentSong.user_id) {
          queries.push(
            supabase.from('public_profiles').select('*').eq('user_id', currentSong.user_id).single()
          );
        }

        // Tracks del álbum (si la canción pertenece a un álbum)
        if (currentSong.album_id) {
          queries.push(
            supabase.from('songs')
              .select('id, title, cover_url, display_artist, audio_url, track_number, duration')
        ...
        // Más canciones del artista (excluye las del álbum actual)
        if (currentSong.user_id) {
          let q = supabase.from('songs')
            .select('id, title, cover_url, display_artist, audio_url')
        ...
        // Siguiente canción: siguiente en la cola
        const queueList = queue || [];
        const currentIdx = queueList.findIndex(s => s.id === currentSong.id);
        if (currentIdx !== -1 && currentIdx < queueList.length - 1) {
          setNextSong(queueList[currentIdx + 1]);
        }
      } catch (e) {
  ...
  const handleMuteToggle = useCallback(() => {
    if (isMuted) {
      handleVolume({ target: { value: prevVolume } });
      setIsMuted(false);
    } else {
  ...
  // ── Helper: is this track the one playing? ──
      ...
      {isFullscreen && (
        ...
        >
          ...
          <div className="fixed inset-0 -z-10 overflow-hidden pointer-events-none">
            <div
              className="absolute inset-0 opacity-50 blur-[120px] scale-150"
              style={{
          ...
          <div className="min-h-screen flex flex-col">
            ...
            <header className="flex-shrink-0 px-6 pt-6 pb-4 flex items-center justify-between">
              ...
              <div className="text-center">
                ...
                <p className="text-sm font-bold mt-0.5">
                  {currentSong.album_title || currentSong.genre || 'Tu Biblioteca'}
                </p>
              </div>
            ...
            {/* ── Main: Portada cuadrada + Controles ── */}
            <main className="flex-1 flex flex-col md:flex-row items-center justify-center gap-8 md:gap-14 px-6 md:px-16 max-w-6xl mx-auto w-full py-4 md:py-8">
              ...
              {/* Portada — cuadrada con sombra y leve scale al reproducir */}
              <motion.div
              ...
              <div className="flex-1 w-full max-w-[480px]">
                ...
                <div className="flex items-start justify-between mb-6">
                  <div className="min-w-0 flex-1">
                    <h1 className="text-3xl md:text-4xl font-black leading-tight line-clamp-2 mb-1">
                      {currentSong.title}
                    </h1>
                  ...
                  <div className="flex items-center gap-1 ml-3 flex-shrink-0">
                    ...
                    <button className="p-2 hover:bg-white/10 rounded-full transition-colors text-white/50 hover:text-white" aria-label="Compartir">
                ...
                <div className="mb-5">
                  <div className="relative h-1 w-full bg-white/20 rounded-full cursor-pointer group">
                    <div
                      className="absolute h-full bg-white rounded-full transition-all group-hover:bg-green-400"
                ...
                <div className="flex items-center justify-between mb-5">
                  <button
                    ...
                    className={`p-2 rounded-full transition-colors hover:bg-white/10 ${shuffle ? 'text-green-400' : 'text-white/50 hover:text-white'}`}
                    aria-label="Shuffle"
                  >
                  ...
                  <button
                    onClick={cycleRepeat}
                    className={`p-2 rounded-full transition-colors hover:bg-white/10 ${repeatMode !== 'none' ? 'text-green-400' : 'text-white/50 hover:text-white'}`}
                    aria-label="Repetir"
                  >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className={`p-2 rounded-full transition-colors hover:bg-white/10 flex-shrink-0 ${showQueue ? 'text-green-400' : 'text-white/50 hover:text-white'}`}
                    aria-label="Cola"
                  >
          ...
          <div className="bg-black/40 backdrop-blur-3xl w-full border-t border-white/5">
            <div className="max-w-4xl mx-auto px-6 py-16 space-y-16">
              ...
              {/* ── Tracks del álbum + Siguiente canción ── */}
                      ...
                      <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden mb-4">
                        {albumTracks.map((track, i) => {
                          ...
                          return (
                            ...
                            >
                              {/* Número de track o equalizer */}
                              <div className="w-6 flex-shrink-0 text-center">
                                {active
                                  ? <EqualizerBars isPlaying={isPlaying} />
                                  : <span className="text-xs text-white/30 group-hover:hidden">{track.track_number || i + 1}</span>
                                }
                                {!active && (
                                  <Play className="w-3 h-3 text-white/60 hidden group-hover:block mx-auto" fill="currentColor" />
                                )}
                              </div>
                              <div className="flex-1 min-w-0">
                                <p className={`text-sm font-semibold truncate ${active ? 'text-green-400' : 'text-white group-hover:text-green-400 transition-colors'}`}>
                                  {track.title}
                                </p>
                                <p className="text-xs text-white/40 truncate">{track.display_artist}</p>
                              </div>
                              {track.duration && (
                                <span className="text-xs text-white/30 flex-shrink-0 tabular-nums">{formatTime(track.duration)}</span>
                              )}
                            </div>
                          );
                        })}
                  ...
                  <div className="flex flex-col md:flex-row gap-8 items-center md:items-start">
                    ...
                    <div className="relative flex-shrink-0">
                      <div className="w-36 h-36 rounded-full overflow-hidden shadow-2xl ring-4 ring-white/10">
                        <img
                          src={artistInfo.avatar_url || coverUrl}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {relatedSongs.map(song => (
                      ...
                      >
                        ...
                        <button
                          onClick={(e) => { e.stopPropagation(); /* añadir a cola */ }}
                          className="p-1.5 rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:bg-white/10 text-white/60 hover:text-white flex-shrink-0"
            ...
            >
              <div className="relative flex-shrink-0">
                <motion.img
                  ...
                  className="w-11 h-11 rounded-lg shadow-md object-cover"
              ...
              <div className="min-w-0">
                <p className="font-bold text-sm text-gray-900 truncate leading-tight group-hover:text-purple-600 transition-colors">
                  {currentSong.title}
                </p>
