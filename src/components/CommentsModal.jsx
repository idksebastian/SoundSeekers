        ...
        <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg flex flex-col max-h-[85vh]">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs text-gray-400">{comments.length} comentario{comments.length !== 1 ? 's' : ''}</p>
            </div>
          ...
          <div className="px-6 py-4 border-b border-gray-100 bg-gray-50">
            <div className="flex items-center gap-2 mb-2">
              <div className="w-7 h-7 rounded-full bg-purple-100 flex items-center justify-center text-purple-700 font-bold text-xs shrink-0 overflow-hidden">
                {post.avatar_url
                  ? <img src={post.avatar_url} alt={post.username} className="w-full h-full object-cover"/>
                  : post.username?.[0]?.toUpperCase() ?? '?'
                }
              </div>
              <span className="text-xs font-semibold text-gray-700">{post.username ?? 'Usuario'}</span>
              <span className="text-xs text-gray-400">{timeAgo(post.created_at)}</span>
            </div>
            <p className={`text-sm text-gray-600 leading-relaxed ${!showFullPost ? 'line-clamp-2' : ''}`}>
              {post.content}
            </p>
            {post.content?.length > 120 && (
              <button
                onClick={() => setShowFullPost(p => !p)}
                    ...
                    {editingId === comment.id ? (
                      <div className="flex flex-col gap-2 mt-1">
                        ...
                        <div className="flex gap-2 justify-end">
                          <button onClick={() => setEditingId(null)} className="text-xs text-gray-400 hover:text-gray-600 transition">
                            Cancelar
                          </button>
